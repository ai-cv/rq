...
def copyRect(src, dst, srcRect, dstRect, mask=None,
    ...
    if mask is None:
        ...
        dst[y1:y1 + h1, x1:x1 + w1] = np.where(cv2.resize(mask, (w1, h1), interpolation= cv2.INTER_NEAREST), cv2.resize(src[y0:y0 + h0, x0:x0 + w0], (w1, h1), interpolation=interpolation), dst[y1:y1 + h1, x1:x1 + w1])
